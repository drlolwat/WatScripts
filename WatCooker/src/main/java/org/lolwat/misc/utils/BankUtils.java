...
public class BankUtils {
    public static void bank(WatTask task) {
        ...
        if (banker == null || !banker.exists()) {
            ...
            if (chest == null) {
                ...
                TaskManager.getInstance().setCurrentTask(new TraversalTask(ItemUtils.getClosestAllowedBank().getArea(3), task));
                return;
            } else {
    ...
    public boolean hasItem(String name, int amount) {
        return Bank.contains(x -> x != null
                && x.getName().startsWith(name)
                && !x.getName().contains("(1)")
                && x.getAmount() >= amount
        );
    }
}
