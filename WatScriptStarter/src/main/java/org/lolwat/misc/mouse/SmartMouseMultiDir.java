...
public class SmartMouseMultiDir implements MouseAlgorithm {
    ...
    private static final double SPEED_VARIANCE = 0.01;
    ...
    private void moveSmoothly(Point start, Point end, double totalMovementSeconds, EasingFunction easingFunc) {
        ...
        // Reduce subdivisions for faster movement
        int subdivisions = Math.max(5, (int) (dist / 3.0));
        ...
        for (int i = 1; i <= subdivisions; i++) {
            ...
            sleep(Math.max(3, microSleepMs)); // Ensure a minimum sleep time
        }
    }
