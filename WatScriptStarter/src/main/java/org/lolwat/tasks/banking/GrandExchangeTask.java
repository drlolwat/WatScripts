                ...
                if (isSelling) {
                    ...
                    while (item.getValue() != 0) {
                        ...
                        if (GrandExchange.addBuyItem(itemFinal)) {
                            ...
                            if (itemCost > coins) {
                                Logger.error("Didn't have enough coins to fulfill offer, going to mule");
                                TaskManager.getInstance().setCurrentTask(new MulingTask("Getting gold", Worlds.getCurrentWorld(), new HashMap<String, Integer>() {
                                    {
                                        put("Coins", itemCost * (item.getValue() >= 1 ? item.getValue() : -item.getValue()));
                                    }
                                }, this));
                                return;
                            }
